...
class GiftCardContract extends contracts.ExpiringStandaloneValueAssetBase {
  /**
  ...
  async Mint(ctx, userId, amount, currency, enforcementDate, expirationDate) {
    ...
    const newAsset = new assets.ExpiringStandaloneValueAsset(
      `${txID}.0`,
      userId,
      assets.types.AssetType.GIFT_CARD,
      amountInt,
      currency,
      enforcementDateInt,
      expirationDateInt,
    ...
    await Promise.all(
      assetOwnersAndKeysEntries.map(async ([assetOwner, assetKeys]) => {
        ...
        resolvedAssets.forEach((asset, index) => {
          ...
          if (burningAssets[assetKey] !== undefined) {
          ...
          burningAssets[assetKey] = asset;
        });
      })
    );

    const txID = ctx.stub.getTxID();
    const burntAssets = this._burn(ctx, Object.values(burningAssets));

    ctx.stub.setEvent(
      "BurnEvent",
      await utils.wrapEvent(ctx, { inputs: Object.values(burningAssets) })
    );
  ...
  async _retrieveAssetByKey(ctx, assetKey) {
    const assetCompositeKey = ctx.stub.createCompositeKey(
      assets.types.AssetType.GIFT_CARD.toString(),
      [assetKey]
    );
...
module.exports.contract = GiftCardContract;
